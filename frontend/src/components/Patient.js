    ...
    const handleSave = async () => {
      try {
        ...
        if (response.ok) {
          ...
          toast.error(`Failed to update patient profile: ${errorData.error}`);
        }
      } catch (error) {
        toast.error("Error updating patient profile. Please try again.");
      }
    };
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          toast.error(`Failed to book appointment: ${errorData.error}`);
        }
      } catch (error) {
